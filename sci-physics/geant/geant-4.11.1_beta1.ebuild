...
MY_P=${PN}$(ver_cut 1)-v$(ver_cut 2-3)

case ${PV} in
*_beta*)
	MY_P=${MY_P}.b$(printf "%02d" $(ver_cut 5))
	DOCS="ReleaseNotes/Beta$(ver_cut 1).$(ver_cut 2-3)-*.txt"
	;;
*)
...
RDEPEND="
	...
	data? ( ~sci-physics/geant-data-4.11.1_beta1 )
...
src_configure() {
	local mycmakeargs=(
		...
		-DGEANT4_BUILD_ENABLE_ASSERTIONS=$(usex debug)
	...
	if use inventor; then
		mycmakeargs+=(
			-DGEANT4_USE_INVENTOR=$(usex qt5 OFF ON)
			-DGEANT4_USE_INVENTOR_QT=$(usex qt5 ON OFF)
		)
	fi
...
src_install() {
	...
	rm -r "${ED}"/usr/$(get_libdir)/Geant4-$(ver_cut 2-3)* || die "failed to remove recursive symlink"
